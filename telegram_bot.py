import asyncio
   ...
   async def websocket_handler(request):
       ...
       try:
           ...
           logger.info(f"Клиент подключен: {request.remote}, всего клиентов: {len(connected_clients)}")
           try:
               async for msg in ws:
                           ...
                           for client in connected_clients.copy():
                               if client != ws and not client.closed:
                                   try:
                                       ...
                                       logger.info(f"Отправлено сообщение клиенту: {client}")
           ...
           logger.error(f"Ошибка при установке WebSocket-соединения: {e}")
   ...
   async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
       ...
       if action:
           logger.info(f"Отправка команды на WebSocket: {action['sound']}, подключенных клиентов: {len(connected_clients)}")
           if not connected_clients:
               logger.warning("Нет подключенных WebSocket-клиентов")
           for client in connected_clients.copy():
               if not client.closed:
                   try:
                       await client.send_str(action["sound"])
                       logger.info(f"Команда {action['sound']} отправлена клиенту")
                   except Exception as e:
                       logger.error(f"Ошибка отправки клиенту: {e}")
                       connected_clients.discard(client)
               else:
                   logger.warning("Клиент закрыт, удаляем из списка")
                   connected_clients.discard(client)
   ...
   async def main():
       ...
       await site.start()
       logger.info(f"WebSocket-сервер запущен на ws://0.0.0.0:{port}/ws")
